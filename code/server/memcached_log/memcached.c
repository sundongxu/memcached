...
static enum try_read_result try_read_network(conn *c) {
    ...
    while (1) {
		...
		LOG_TRACE_WITH_PARAMS("try_read_network()---read() invoked---res:%d", res);
		LOG_TRACE_WITH_PARAMS("read() invoked---c->rbuf+c->rbytes:%s", c->rbuf+c->rbytes);
		if (res > 0) {
            pthread_mutex_lock(&c->thread->stats.mutex);
            c->thread->stats.bytes_read += res;
            pthread_mutex_unlock(&c->thread->stats.mutex);
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
		...
		LOG_TRACE("case conn_nread...read() invoked!");
                if (res > 0) {
	    ...
	    LOG_TRACE_WITH_PARAMS("case conn_nread...---res:%d, errno:%d\n", res, errno);
            ...
            if (res == -1 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
	    LOG_TRACE("update_event() from case conn_nread...");
                if (!update_event(c, EV_READ | EV_PERSIST)) {
                    if (settings.verbose > 0)
                        fprintf(stderr, "Couldn't update event\n");
                    conn_set_state(c, conn_closing);
...
static int server_socket(const char *interface,
    ...
    for (next= ai; next; next= next->ai_next) {
        ...
        if (bind(sfd, next->ai_addr, next->ai_addrlen) == -1) {
            ...
            success++;
            LOG_TRACE_WITH_PARAMS("%d listen socket existed now, current listen sfd:%d", success, sfd);
