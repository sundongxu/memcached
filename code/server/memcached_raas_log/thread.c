 ...
 */
static void thread_libevent_process(int fd, short which, void *arg) {
    LOG_TRACE("thread_libevent_process() invoked!");
    LIBEVENT_THREAD *me = arg;
    CQ_ITEM *item;
    char buf[1];
    conn *c;
...
void dispatch_conn_new(int sfd, enum conn_states init_state, int event_flags,
    ...
    LOG_TRACE("dispatch_conn_new() invoked!");
...
item *item_alloc(char *key, size_t nkey, int flags, rel_time_t exptime, int nbytes) {
    LOG_TRACE_WITH_PARAMS("item_alloc() invoked!---key:%s", key);
    item *it;
...
item *item_get(const char *key, const size_t nkey, conn *c, const bool do_update) {
    LOG_TRACE_WITH_PARAMS("item_get() invoked!---key:%s", key);
    item *it;
    uint32_t hv;
    hv = hash(key, nkey);
    LOG_TRACE_WITH_PARAMS("item_get() invoked!---key:%s, nkey:%zd, hashvalue:%u", key, nkey, hv);    
    item_lock(hv);
...
int item_replace(item *old_it, item *new_it, const uint32_t hv) {
    LOG_TRACE("item_replace() invoked!");
...
void memcached_thread_init(int nthreads) {
    LOG_TRACE("memcached_thread_init() invoked!");
    int         i;
    int         power;
    ...
    for (i = 0; i < nthreads; i++) {
        ...
        LOG_TRACE_WITH_PARAMS("setup_thread() invoked! current thread:%d", i);
    ...
    for (i = 0; i < nthreads; i++) {
        LOG_TRACE_WITH_PARAMS("create_worker() invoked! current thread:%d", i);
