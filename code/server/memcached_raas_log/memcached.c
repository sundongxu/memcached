...
static int add_msghdr(conn *c)
{
    // Add message header to a connection for transmitting
    struct msghdr *msg;
...
{
    LOG_TRACE("conn_release_items() invoked!");
    assert(c != NULL);
...
{
    LOG_TRACE("conn_cleanup() invoked!");
    assert(c != NULL);
...
{
    LOG_TRACE_WITH_PARAMS("out_string() invoked!---str:%s", str);
    size_t len;
    ...
    c->msgused = 0;
    c->iovused = 0;
    add_msghdr(c); // add a message header to a connection
...
{
    LOG_TRACE("complete_nread_ascii() invoked!");
    assert(c != NULL);
...
{
    LOG_TRACE("reset_cmd_handler() invoked!");
    c->cmd = -1;
...
static void complete_nread(conn *c)
{
    LOG_TRACE("complete_nread() invoked!");
    assert(c != NULL);
...
{
    LOG_TRACE("do_store_item() invoked!");
...
{
    LOG_TRACE_WITH_PARAMS("limited_get() invoked!---key:%s", key);
...
{
    LOG_TRACE("process_get_command() invoked!");
    char *key;
    size_t nkey;
    int i = 0;
    item *it;
...
{
    LOG_TRACE("process_update_command() invoked!");
    char *key;
    size_t nkey;
    unsigned int flags;
...
static void process_command(conn *c, char *command)
{
    LOG_TRACE_WITH_PARAMS("process_command() invoked!---command:%s", command);
    ...
    LOG_TRACE_WITH_PARAMS("process_command()---command type:%s", tokens[COMMAND_TOKEN].value);
    if (ntokens >= 3 &&
...
static int try_read_command(conn *c)
{
    LOG_TRACE("try_read_command() invoked!");
    assert(c != NULL);
...
static enum try_read_result try_read_udp(conn *c)
{
    LOG_TRACE("try_read_udp() invoked!");
    int res;
...
static enum try_read_result try_read_network(conn *c)
{
    // 可能二次读到空数据，ret=READ_ERROR
    LOG_TRACE("try_read_network() invoked!");
    ...
    while (1)
    {
        ...
        // To be replaced by RaaS User Interface
        // res = read(c->sfd, c->rbuf + c->rbytes, avail);
        res = rrecv(c->sfd, c->rbuf + c->rbytes, avail, 0);
        LOG_TRACE_WITH_PARAMS("try_read_network()---rrecv() invoked!---res:%d", res);
		LOG_TRACE_WITH_PARAMS("rrecv() invoked!---c->buf + c->rbytes:%s, 后移19字节:%s，avail:%d", c->rbuf + c->rbytes, c->rbuf+c->rbytes+19, avail);
        if (res > 0)
        {
            pthread_mutex_lock(&c->thread->stats.mutex);
            c->thread->stats.bytes_read += res;
...
static bool update_event(conn *c, const int new_flags)
{
    LOG_TRACE("update_event() invoked!");
    assert(c != NULL);
...
{
    LOG_TRACE("do_accept_new_conns() invoked!");
    conn *next;
...
static enum transmit_result transmit(conn *c)
{
    ...
    if (c->msgcurr < c->msgused)
    {
        ...
        res = rsendmsg(c->sfd, m, 0);
        LOG_TRACE_WITH_PARAMS("rsendmsg() invoked!---res:%zd", res);
        if (res > 0)
        {
            pthread_mutex_lock(&c->thread->stats.mutex);
...
{
    LOG_TRACE("read_into_chunked_item() invoked!");
    int total = 0;
    int res;
    ...
    while (c->rlbytes > 0)
    {
        ...
        else
        {
            ...
            res = rrecv(c->sfd, ch->data + ch->used,
                        (unused > c->rlbytes ? c->rlbytes : unused), 0);
            LOG_TRACE_WITH_PARAMS("read_into_chunked_item()---rrecv() invoked!---res:%d", res);
            if (res > 0)
            {
                pthread_mutex_lock(&c->thread->stats.mutex);
                c->thread->stats.bytes_read += res;
...
static void drive_machine(conn *c)
{
    LOG_TRACE("drive_machine() invoked!");
    bool stop = false;
    int sfd;
//    socklen_t addrlen;
...
//    const char *str;
    ...
    while (!stop)
    {
        ...
        {
        case conn_listening:
            LOG_TRACE("case conn_listening...");
//            addrlen = sizeof(addr);
//#ifdef HAVE_ACCEPT4
//            // To be replaced by Raas user interface
//            if (use_accept4)
//            {
//                LOG_TRACE("use accept4!");
//                sfd = accept4(c->sfd, (struct sockaddr *)&addr, &addrlen, SOCK_NONBLOCK);
//            }
//            else
//            {
//                LOG_TRACE("use accept!");
//                sfd = accept(c->sfd, (struct sockaddr *)&addr, &addrlen);
//            }
//#else
//            sfd = accept(c->sfd, (struct sockaddr *)&addr, &addrlen);
//#endif
	    sfd = raccept(c->sfd, (struct sockaddr *)&addr, 0);
            LOG_TRACE_WITH_PARAMS("accept fd:%d", sfd);
            if (sfd == -1)
            {
                ...
                stats_state.curr_conns + stats_state.reserved_fds >= settings.maxconns - 1)
            {
                ...
                res = rsend(sfd, str, strlen(str), 0);
                close(sfd);
                STATS_LOCK();
        ...
        case conn_read:
            LOG_TRACE("case conn_read...");
            res = IS_UDP(c->transport) ? try_read_udp(c) : try_read_network(c);
            switch (res)
            {
            case READ_NO_DATA_RECEIVED:
                LOG_TRACE("try_read_network() result:READ_NO_DATA_RECEIVED");
                conn_set_state(c, conn_waiting);
                break;
            case READ_DATA_RECEIVED:
                LOG_TRACE("try_read_network() result:READ_DATA_RECEIVED");
                conn_set_state(c, conn_parse_cmd);
                break;
            case READ_ERROR:
                LOG_TRACE("try_read_network() result:READ_ERROR");
                conn_set_state(c, conn_closing);
                break;
            case READ_MEMORY_ERROR: /* Failed to allocate more memory */
                /* State already set by try_read_network */
                LOG_TRACE("try_read_network() result:READ_MEMORY_ERROR");
                break;
            }
            break;

        case conn_parse_cmd:
            LOG_TRACE("case conn_parse_cmd...");
        ...
        case conn_nread:
            ...
            if (!c->item || (((item *)c->item)->it_flags & ITEM_CHUNKED) == 0)
            {
                ...
                res = rrecv(c->sfd, c->ritem, c->rlbytes, 0);
                LOG_TRACE_WITH_PARAMS("case conn_nread...rrecv() invoked!---res:%d", res);
				LOG_TRACE_WITH_PARAMS("rrecv() invoked!---c->ritem:%s, c->rlbytes:%d", c->ritem, c->rlbytes);
				if (res > 0)
                {
                    LOG_TRACE_WITH_PARAMS("case conn_nread...----res:%d", res);
                    pthread_mutex_lock(&c->thread->stats.mutex);
                    c->thread->stats.bytes_read += res;
                    pthread_mutex_unlock(&c->thread->stats.mutex);
                    if (c->rcurr == c->ritem)
	    ...
	    LOG_TRACE_WITH_PARAMS("case conn_nread...res:%d, errno:%d\n", res, errno);
            if (res == 0)
            { /* end of stream */
                conn_set_state(c, conn_closing);
                break;
            ...
            if (res == -1 && (errno == EAGAIN || errno == EWOULDBLOCK))
            {
                if (!update_event(c, EV_READ | EV_PERSIST))
                {
		    LOG_TRACE("case conn_nread...update_event() return false!");
                    if (settings.verbose > 0)
                        fprintf(stderr, "Couldn't update event\n");
                    conn_set_state(c, conn_closing);
                    break;
        ...
        case conn_swallow:
            ...
            res = rrecv(c->sfd, c->rbuf, c->rsize > c->sbytes ? c->sbytes : c->rsize, 0);
            LOG_TRACE_WITH_PARAMS("case conn_swallow...rrecv() invoked!---res:%d", res);
            if (res > 0)
            {
                pthread_mutex_lock(&c->thread->stats.mutex);
                c->thread->stats.bytes_read += res;
        ...
        case conn_mwrite:
            ...
            case TRANSMIT_COMPLETE:
                LOG_TRACE("transmit() result:TRANSMIT_COMLETE");
        ...
        case conn_closed:
            LOG_TRACE("case conn_closed...");
            /* This only happens if dormando is an idiot. */
            abort();
            break;

        case conn_watch:
            LOG_TRACE("case conn_watch...");
            /* We handed off our connection to the logger thread. */
            stop = true;
            break;
        case conn_max_state:
            LOG_TRACE("case conn_max_state...");
            assert(false);
            break;
        }
    }
...
{
    LOG_TRACE("event_handler(() invoked!");
    conn *c;
...
//{
//    LOG_TRACE("maximize_sndbuf() invoked!");
...
{
    LOG_TRACE("server_socket() invoked!");
    int sfd;
...
//    hints.ai_socktype = IS_UDP(transport) ? SOCK_DGRAM : SOCK_STREAM;
    ...
    // {
    //     LOG_TRACE("server_socket() next loop!");
    // add by roysun
//    next = ai;
    ...
    // add by roysun
    int handle = 0;
    gettargetInstance(&handle);
    char ip_buf[NI_MAXHOST];
    snprintf(ip_buf, sizeof(ip_buf), "%s", interface);
    char params[NI_MAXHOST + NI_MAXSERV];
    snprintf(params, sizeof(params), "%s:%s", ip_buf, port_buf);
    rparse(handle, params);
    // end addition
    LOG_TRACE_WITH_PARAMS("rlisten() invoked!---IP:port => %s\n", params);
    if ((sfd = rlisten(handle, 0)) == -1)
    {
        perror("rlisten():sfd == -1");
        return 1;
    }
    else
    {
    ...
    // LOG_TRACE_WITH_PARAMS("%d listen socket existed now, current listen sfd:%d", sfd);
    LOG_TRACE_WITH_PARAMS("current rlisten fd:%d", sfd);
...
static int server_sockets(int port, enum network_transport transport,
                          FILE *portnumber_file)
{
    LOG_TRACE("server_sockets() invoked!");
...
{
    LOG_TRACE("new_socket_unix() invoked!");
    int sfd;
    int flags;
...
{
    LOG_TRACE("server_socket_unix() invoked!");
    int sfd;
    struct linger ling = {0, 0};
...
//        }
    ...
    {
        LOG_TRACE("event_base_loop() invoked!");
        retval = EXIT_FAILURE;
    }
