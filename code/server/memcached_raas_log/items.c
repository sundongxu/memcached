...
static item *do_item_alloc_pull(const size_t ntotal, const unsigned int id) {
    LOG_TRACE("do_item_alloc_pull() invoked!");
    item *it = NULL;
    int i;
...
item_chunk *do_item_alloc_chunk(item_chunk *ch, const size_t bytes_remain) {
    LOG_TRACE("do_item_alloc_chunk() invoked!");
...
item *do_item_alloc(char *key, const size_t nkey, const unsigned int flags,
    ...
    LOG_TRACE_WITH_PARAMS("do_item_alloc() invoked!---key:%s", key);
    uint8_t nsuffix;
    item *it = NULL;
    char suffix[40];
...
int do_item_link(item *it, const uint32_t hv) {
    LOG_TRACE("do_item_link() invoked!");
...
void do_item_unlink(item *it, const uint32_t hv) {
    LOG_TRACE("do_item_unlink() invoked!"); 
...
void do_item_remove(item *it) {
    LOG_TRACE("do_item_remove() invoked!");
...
int do_item_replace(item *it, item *new_it, const uint32_t hv) {
    ...
    LOG_TRACE("do_item_replace() invoked!");
    assert((it->it_flags & ITEM_SLABBED) == 0);
